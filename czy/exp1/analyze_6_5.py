import numpy as np
import pandas as pd
from scipy import signal

df = pd.read_csv(r'e:\X1\real_test\origin_exp1\agibot_x1_train-main\czy\data\isaac_diag_20260624_155216.csv')
print(f"Shape: {df.shape}")

def joint_range(arr):
    return arr.max() - arr.min()

joints = ['hip_pitch', 'hip_roll', 'hip_yaw', 'knee_pitch', 'ankle_pitch', 'ankle_roll']

print("=" * 70)
print("【实验 6.5 诊断】isaac_diag_20260624_155216.csv")
print("=" * 70)

# 1. 速度
print("\n─── 1. 速度跟踪 ───")
cmd = df['cmd_linear_x'].values
blv = df['base_lin_vel_x'].values
print(f"  cmd_linear_x 均值: {np.mean(cmd):.3f}")
print(f"  base_lin_vel_x 均值: {np.mean(blv):.3f}")
print(f"  base_lin_vel_x std: {np.std(blv):.3f}")

# 2. 关节范围
print("\n─── 2. 关节实际范围 ───")
for j in joints:
    l = df[f'pos_left_{j}_joint'].values
    r = df[f'pos_right_{j}_joint'].values
    print(f"  {j:15s}: 左={joint_range(l):.4f}  右={joint_range(r):.4f}")

# 3. 关节均值
print("\n─── 3. 关节均值（对比默认值）───")
defaults = {
    'hip_pitch': (0.4, -0.4),
    'hip_roll': (0.05, -0.05),
    'hip_yaw': (-0.31, 0.31),
    'knee_pitch': (0.49, 0.49),
    'ankle_pitch': (-0.21, -0.21),
    'ankle_roll': (0.0, 0.0),
}
for j in joints:
    l = df[f'pos_left_{j}_joint'].values
    r = df[f'pos_right_{j}_joint'].values
    dl, dr = defaults[j]
    print(f"  {j:15s}: 左={np.mean(l):+.4f}(默认{dl:+.2f})  右={np.mean(r):+.4f}(默认{dr:+.2f})")

# 4. hip_roll/yaw 偏移检查
print("\n─── 4. hip_roll/yaw 偏移检查 ───")
for j in ['hip_roll', 'hip_yaw']:
    l = df[f'pos_left_{j}_joint'].values
    r = df[f'pos_right_{j}_joint'].values
    dl, dr = defaults[j]
    l_offset = np.abs(np.mean(l) - dl)
    r_offset = np.abs(np.mean(r) - dr)
    print(f"  {j}: 左偏移={l_offset:.4f}  右偏移={r_offset:.4f}")

# 5. 抖动指标
print("\n─── 5. 抖动指标 ───")
for j in ['knee_pitch', 'hip_pitch']:
    l = df[f'pos_left_{j}_joint'].values
    r = df[f'pos_right_{j}_joint'].values
    fs = 100; n = len(l)
    freqs, psd_l = signal.welch(l, fs=fs, nperseg=min(2000, n))
    _, psd_r = signal.welch(r, fs=fs, nperseg=min(2000, n))
    hf_l = np.sum(psd_l[freqs>5])/np.sum(psd_l)*100
    hf_r = np.sum(psd_r[freqs>5])/np.sum(psd_r)*100
    print(f"  {j}: 左HF={hf_l:.1f}% 右HF={hf_r:.1f}%")

# 6. 接触状态
print("\n─── 6. 接触状态 ───")
lc = df['left_contact'].values
rc = df['right_contact'].values
print(f"  左脚接触率: {np.mean(lc>0)*100:.1f}%")
print(f"  右脚接触率: {np.mean(rc>0)*100:.1f}%")
print(f"  双脚同时接触率: {np.mean((lc>0)&(rc>0))*100:.1f}%")

# 7. 躯干稳定性
print("\n─── 7. 躯干稳定性 ───")
print(f"  base_ang_vel_x std: {np.std(df['base_ang_vel_x'].values):.4f}")
print(f"  base_euler_x 范围: {joint_range(df['base_euler_x'].values):.4f}")

# 8. Action 输出
print("\n─── 8. Action 输出范围 ───")
for j in ['hip_pitch', 'knee_pitch']:
    l = df[f'action_left_{j}_joint'].values
    r = df[f'action_right_{j}_joint'].values
    print(f"  {j:15s}: 左={joint_range(l):.4f}  右={joint_range(r):.4f}")

# 9. 历史对比
print("\n─── 9. 历史对比 ───")
print(f"{'指标':<25s} {'6.3':>10s} {'6.4':>10s} {'6.5':>10s}")
print("-" * 60)
print(f"{'速度vx均值':<25s} {'0.430':>10s} {'0.368':>10s} {np.mean(blv):>10.3f}")
print(f"{'左膝范围':<25s} {'0.303':>10s} {'0.673':>10s} {joint_range(df['pos_left_knee_pitch_joint'].values):>10.3f}")
print(f"{'右膝范围':<25s} {'0.333':>10s} {'0.531':>10s} {joint_range(df['pos_right_knee_pitch_joint'].values):>10.3f}")

l = df['pos_left_knee_pitch_joint'].values
r = df['pos_right_knee_pitch_joint'].values
fs = 100; n = len(l)
freqs, psd = signal.welch(l, fs=fs, nperseg=min(2000, n))
hf_l = np.sum(psd[freqs>5])/np.sum(psd)*100
_, psd = signal.welch(r, fs=fs, nperseg=min(2000, n))
hf_r = np.sum(psd[freqs>5])/np.sum(psd)*100
print(f"{'左膝HF能量':<25s} {'12.2%':>10s} {'1.3%':>10s} {hf_l:>9.1f}%")
print(f"{'右膝HF能量':<25s} {'11.1%':>10s} {'1.3%':>10s} {hf_r:>9.1f}%")

lr = joint_range(df['pos_left_knee_pitch_joint'].values)
rr = joint_range(df['pos_right_knee_pitch_joint'].values)
print(f"{'左右膝范围比':<25s} {'1.1:1':>10s} {'1.3:1':>10s} {lr/max(rr,0.001):>9.1f}:1")

l = df['pos_left_hip_yaw_joint'].values
r = df['pos_right_hip_yaw_joint'].values
print(f"{'左hip_yaw偏移':<25s} {'—':>10s} {'0.71':>10s} {np.abs(np.mean(l)-defaults['hip_yaw'][0]):>10.3f}")
print(f"{'右hip_roll偏移':<25s} {'—':>10s} {'0.31':>10s} {np.abs(np.mean(r)-defaults['hip_roll'][1]):>10.3f}")
