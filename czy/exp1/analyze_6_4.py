...
df = pd.read_csv(r'e:\X1\real_test\origin_exp1\agibot_x1_train-main\czy\data\isaac_diag_20260624_122059.csv')
...
print("=" * 70)
print("【实验 6.4 诊断】isaac_diag_20260624_122059.csv")
print("=" * 70)
...
# 10. 对比历史
print("\n─── 10. 历史对比 ───")
print(f"{'指标':<25s} {'6.1':>10s} {'6.3':>10s} {'6.4':>10s}")
print("-" * 60)
print(f"{'速度vx均值':<25s} {'0.258':>10s} {'0.430':>10s} {np.mean(blv):>10.3f}")
print(f"{'左膝范围':<25s} {'0.460':>10s} {'0.303':>10s} {joint_range(df['pos_left_knee_pitch_joint'].values):>10.3f}")
print(f"{'右膝范围':<25s} {'0.282':>10s} {'0.333':>10s} {joint_range(df['pos_right_knee_pitch_joint'].values):>10.3f}")
print(f"{'左膝HF能量':<25s} {'0.4%':>10s} {'12.2%':>10s}", end="")
l = df['pos_left_knee_pitch_joint'].values
fs = 100; n = len(l)
freqs, psd = signal.welch(l, fs=fs, nperseg=min(2000, n))
hf = np.sum(psd[freqs>5])/np.sum(psd)*100
print(f" {hf:>9.1f}%")
print(f"{'右膝HF能量':<25s} {'3.3%':>10s} {'11.1%':>10s}", end="")
...
print(f"{'左右膝范围比':<25s} {'1.6:1':>10s} {'1.1:1':>10s} {lr/max(rr,0.001):>9.1f}:1")
